...
public class QuestCatchPig extends Quest {
	...
	public QuestCatchPig(GameMapScene map) {
		...
		this.conversation.add("Hi, my name is Detlef. I need to pass the canyon, can you help me?");
		this.conversation.add("Hmm... My back hurts, I'm so old!");
		this.conversation.add("And even worse, two of my pigs broke out this morning...");
		this.conversation.add("I'm scared that the dinosaurs will kill them... Can you catch them for me?");
		this.conversation.add("I will give you my axe so you can destroy the trees and go trought the canyon.");
		...
		pig2.setMoveStrategy(new RandomMoveStrategy(pig2, 100, 300, 0, 42*32, 65*32, 11*32, 43*32));
	...
	public void onFinish() {
		ResourcesManager.getInstance().activity.toastOnUIThread("Thank you very much young man! I'm so lucky...", Toast.LENGTH_LONG);
		ResourcesManager.getInstance().activity.toastOnUIThread("Go grab your axe!", Toast.LENGTH_LONG);
		ResourcesManager.getInstance().activity.toastOnUIThread("Be safe...", Toast.LENGTH_LONG);
		Body b = caveman.getBody();
		Vector2 v = Vector2Pool.obtain(b.getPosition());
		b.setTransform(v.x-3f, v.y-2f, b.getAngle());
		pig.setMoveStrategy(new SimpleMoveStrategy(pig, new Vector2(58*32, 43*32), GameState.WALKING));
		pig2.setMoveStrategy(new SimpleMoveStrategy(pig, new Vector2(55*32, 44*32), GameState.WALKING));
	}
