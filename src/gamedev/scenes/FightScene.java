...
public class FightScene extends Scene {
	...
	public FightScene(final AnimatedObject object) {
		...
		this.object = object;
		int width = (int) this.resourcesManager.avatar.getBody().getPosition().x * 32 - FIGHTBAR_WIDTH/2;
		int height = (int) this.resourcesManager.avatar.getBody().getPosition().y * 32 - 50;
		this.fightBar = new Sprite(width, height, this.resourcesManager.fightRegion, this.resourcesManager.vbom);
//		this.fightBar.setScale(0.7f);
		this.spear = new Sprite(width, height, this.resourcesManager.spearRegion, this.resourcesManager.vbom);
		...
		this.resourcesManager.engine.runOnUpdateThread(new Runnable() {
			...
			public void run() {
				...
				spearBody = PhysicsFactory.createBoxBody(resourcesManager.physicsWorld, spear, BodyType.KinematicBody, PhysicsFactory.createFixtureDef(0, 0, 0, true));
				resourcesManager.physicsWorld.registerPhysicsConnector(new PhysicsConnector(spear, spearBody, true, false));
	...
	public void onManagedUpdate(float pSecondsElapsed) {
		...
		if (this.object.getState() == GameState.DEAD) {
			...
			this.resourcesManager.level.clearChildScene();
		}
	}
